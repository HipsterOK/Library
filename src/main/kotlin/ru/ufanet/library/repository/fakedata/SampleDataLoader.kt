...
class SampleDataLoader(
    ...
    override fun run(vararg args: String?) {
        ...
        val bookCopies = IntStream.rangeClosed(1, 100).mapToObj { it -> BookCopy(
            it.toLong(), paperBooks[it-1], faker.address().zipCode(), faker.date().birthday(), libraryOffices[it-1], 3) }.toList()
        ...
        val users = IntStream.rangeClosed(1, 100).mapToObj { it -> CorpUser(
            ...
            faker.name().fullName(), "123", UserRole.ADMIN, faker.name().name(), faker.name().username(), faker.name().nameWithMiddle()) }.toList()
